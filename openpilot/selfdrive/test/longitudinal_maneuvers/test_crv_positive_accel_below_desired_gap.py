...
class TestCrvPositiveAccelBelowDesiredGap(OpenpilotTestCase):
  ...
  def test_continuously_tracked_lead_does_not_accelerate_below_desired_gap():
    ...
    for _ in range(30):
      ...
      if d_rel < desired_gap and plant.planner.output_a_target > 0.0:
        unsafe_outputs.append((d_rel, desired_gap, plant.planner.output_a_target))

    assert unsafe_outputs, "fixture no longer reaches the below-desired-gap condition"
